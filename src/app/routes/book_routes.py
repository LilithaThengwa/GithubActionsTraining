...
@router.get(f"{prefix_string}/read/{{book_id}}")
...
@router.put(f"{prefix_string}/update/{{book_id}}")
...
@router.delete(f"{prefix_string}/delete/{{book_id}}")
def delete_book(book_id: int):
    if not service.delete_book(book_id):
        raise HTTPException(status_code=404, detail="Book not found")
    return {"message": "Book deleted successfully"}

@router.get(f"{prefix_string}/all")
def read_all_books():
